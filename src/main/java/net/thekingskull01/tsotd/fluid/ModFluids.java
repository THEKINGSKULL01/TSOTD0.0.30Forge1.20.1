...
import net.thekingskull01.tsotd.datagen.ModFluidTagsProvider;
import net.thekingskull01.tsotd.fluidtypes.ModFluidTypes;
import net.thekingskull01.tsotd.fluidtypes.custom.GlowstoneFluidType;
import net.thekingskull01.tsotd.item.TSOTDBucketItem;
import net.thekingskull01.tsotd.util.ConsumingShapelessRecipe;

public class ModFluids {

    public static final DeferredRegister<RecipeSerializer<?>> RECIPE_SERIALIZERS = DeferredRegister.create(ForgeRegistries.Keys.RECIPE_SERIALIZERS, TSOTD.MOD_ID);
    public static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(ForgeRegistries.BLOCKS, TSOTD.MOD_ID);
    public static final DeferredRegister<Item> ITEMS = DeferredRegister.create(ForgeRegistries.ITEMS, TSOTD.MOD_ID);
    public static final DeferredRegister<FluidType> FLUIDTYPES = DeferredRegister.create(ForgeRegistries.Keys.FLUID_TYPES, TSOTD.MOD_ID);
    public static final DeferredRegister<Fluid> FLUIDS = DeferredRegister.create(ForgeRegistries.FLUIDS, TSOTD.MOD_ID);
    public static final DeferredRegister<EntityType<?>> ENTITY_TYPES = DeferredRegister.create(ForgeRegistries.ENTITY_TYPES, TSOTD.MOD_ID);
    ...
    public static final RegistryObject<RecipeSerializer<?>> CONSUMING_SHAPELESS = RECIPE_SERIALIZERS.register("crafting_shapeless_consuming",
            () -> new ConsumingShapelessRecipe.Serializer());


    public static List<FluidStuff> fluidList = new ArrayList<FluidStuff>();

    public static FluidStuff addFluid(String localizedName, ModFluidTypes.TSOTDInfo info, Block.Properties properties,
                                      BiFunction<FluidType.Properties, ModFluidTypes.TSOTDInfo, FluidType> type, BiFunction<Supplier<?
            extends FlowingFluid>, BlockBehaviour.Properties, LiquidBlock> block, Function<ForgeFlowingFluid.Properties,
            ForgeFlowingFluid.Source> source, Function<ForgeFlowingFluid.Properties, ForgeFlowingFluid.Flowing> flowing,
    ...
    public static FluidStuff addFluid(String localizedName, ModFluidTypes.TSOTDInfo info, Block.Properties properties, BiFunction<FluidType.Properties,
            ModFluidTypes.TSOTDInfo, FluidType> type, BiFunction<Supplier<? extends FlowingFluid>, BlockBehaviour.Properties, LiquidBlock> block,
                                      @Nullable Consumer<ForgeFlowingFluid.Properties> fluidProperties, FluidType.Properties prop) {
    ...
    public static final FluidStuff GLOWSTONE = addFluid("Glowstone", new ModFluidTypes.TSOTDInfo("glowstone",
                    0xA5ebd152, 1.5F, 2.0F), Block.Properties.of().mapColor(MapColor.WOOL).noCollission()
                    .replaceable().liquid().pushReaction(PushReaction.DESTROY).noLootTable(), GlowstoneFluidType::new, LiquidBlock::new,
   ...
   //                .motionScale(-0.04D));
    ...
    public static <T extends Entity> RegistryObject<EntityType<T>> register(String name, Builder<T> builder) {
        return ENTITY_TYPES.register(name, () -> builder.build(TSOTD.MOD_ID + ":" + name));
    }

    //public static final RegistryObject<EntityType<FallingSillyPuttyEntity>> FALLING_SILLY_PUTTY = register("falling_silly_putty",
    //        Builder.<FallingSillyPuttyEntity>of(FallingSillyPuttyEntity::new, MobCategory.MISC).sized(0.98F, 0.98F)
    ...
    public static void registerFluidInteractions() {
        FluidInteractionRegistry.addInteraction(ForgeMod.LAVA_TYPE.get(), new InteractionInformation(
                (level, currentPos, relativePos, currentState) -> level.getFluidState(relativePos).is(ModFluidTagsProvider.WATERY),
                fluidState -> fluidState.isSource() ? Blocks.GLOWSTONE.defaultBlockState() : Blocks.PACKED_ICE.defaultBlockState()
        ));
    }
    ...
    public static class FluidStuff {
        ...
        public FluidStuff(String name, String localizedName, int color, FluidType type, BiFunction<Supplier<? extends FlowingFluid>,
                BlockBehaviour.Properties, LiquidBlock> block, @Nullable Consumer<ForgeFlowingFluid.Properties> fluidProperties,
                          Function<ForgeFlowingFluid.Properties, ForgeFlowingFluid.Source> source, Function<ForgeFlowingFluid.Properties,
                ForgeFlowingFluid.Flowing> flowing, Block.Properties properties) {
            ...
            FLUID_BLOCK = BLOCKS.register(name + "_block", () -> block.apply(FLUID, properties.lightLevel((state) -> { return type.getLightLevel();
            }).randomTicks().strength(100.0F).noLootTable()));
            FLUID_BUCKET = ITEMS.register(name + "_bucket", () -> new TSOTDBucketItem(FLUID, new BucketItem.Properties().craftRemainder(Items.BUCKET)
                    .stacksTo(1)));
